...
local function run_project()
  ...
  local bin_path = project_dir .. "/build/bin/" .. app_exe
...
local function debug_project()
  ...
  local cmd = {"ghostty", "-e", "/home/marko/programs/nnd/nnd"};

  local path = vim.fn.expand("%:p")      -- full path
  local line = vim.fn.line(".")          -- current line number

  if vim.endswith(path, ".zig") then
    table.insert(cmd, "--breakpoint")
    table.insert(cmd, path .. ":" .. line);
  end

  local bin_path = project_dir .. "/build/bin/" .. app_exe
  table.insert(cmd, bin_path)

  --vim.fn.jobstart({
    --"ghostty", "-e", "/home/marko/programs/nnd/nnd", "--breakpoint", breakpoint, bin_path
  --}, { detach = true })
